 ...
 * </pre>
 *
 * @author Shyam Sai Bethian and Yihone Chu
 *
 */
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void exchangeEntries(T[] array, int i, int j) {
        ...
        /*
         * If i and j are not equal, then we switch them in the array.
         */
        if (i != j) {
            T tmp = array[i];
            array[i] = array[j];
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        /*
         * First gets the index of both left and right.
         */
        int left = 2 * top + 1;
        int right = 2 * top + 2;

        /*
         * If right is less than last, then that means a left and right node
         * must exist.
         */
        if (right <= last) {

            /*
             * If the value at the left node is smaller than the value at the
             * top node and the right node, then the top node and left node
             * switch. Then we recursively call swiftDown to turn the rest of
             * the tree into a heap.
             */
            if (order.compare(array[left], array[top]) < 0
                ...
                /*
                 * If the value at the left node is bigger than the value at the
                 * top node, but the right node is smaller than the top node,
                 * then the top node and right node switch. Then we recursively
                 * call swiftDown to turn the rest of the tree into a heap.
                 */
            ...
            /*
             * If only the left node exists and there is no right node, then we
             * compare the left and top node. If the left node is smaller, then
             * the top and left node switch, and we call swiftDown to
             * recursively turn the rest of the tree into the heap.
             */
            if (order.compare(array[left], array[top]) < 0) {
                exchangeEntries(array, left, top);
                siftDown(array, left, last, order);
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        /*
         * If right is less than array length, then that means a left and right
         * node must exist.
         */
        if (right < array.length) {
            /*
             * We recursively call heapify on the left node and right node to
             * heapify the rest of the tree.
             */
            heapify(array, left, order);
            heapify(array, right, order);
        } else if (left < array.length && right >= array.length) {
            /*
             * If only the left node is smaller than the array length, then we
             * recursivly call heapify only on the left node to heapify the rest
             * of the tree.
             */
            heapify(array, left, order);
        }
        ...
        /*
         * Calls siftDown to create the heap using the array.
         */
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        /*
         * This while loop fills in each index of the array of elements from the
         * queue q.
         */
        int counter = 0;
        while (q.length() > 0) {
            heap[counter] = q.dequeue();
            counter++;
        }
        /*
         * The resulting array is heapified when calling heapify on it.
         */
    ...
    private void createNewRep(Comparator<T> order) {
        ...
        /*
         * A new representation starts with insertionMode equal to true, 0
         * entries, and a heapSize of 0.
         */
    ...
    public final void add(T x) {
        ...
        /*
         * Enqueues x to the this SortingMachines' entries queue.
         */
    ...
    public final void changeToExtractionMode() {
        ...
        /*
         * Changes insertionMode to false, and the heapSize of this is equal to
         * the entries the length.
         */
        this.insertionMode = false;
        this.heapSize = this.entries.length();

        /*
         * this.heap becomes the heap built using buildHeap with the entries and
         * the comparator machineOrder.
         */
    ...
    public final T removeFirst() {
        ...
        /*
         * The answer is the first entry of the array this.heap.
         */
        T answer = this.heap[0];

        /*
         * Then we exchange the top node with the last entry in this.heap
         */
        exchangeEntries(this.heap, 0, this.heapSize - 1);
        /*
         * After decreasing the value of this.heapSize, we use siftDown to
         * create a heap without the last entry of this.heap, which is the value
         * which we "removed".
         */
        this.heapSize--;
    ...
    public final boolean isInInsertionMode() {
        assert this.conventionHolds();
        /*
         * Returns if this is in Insertion Mode.
         */
    ...
    public final int size() {
        ...
        /*
         * If the machine is in insertionMode, then the answer is the length of
         * the entries. If not, then the answer is the size of the heap.
         */
        int answer = 0;
        if (this.insertionMode) {
